...
app.post('/postMacAddressAndQrcode', function (req, res) {
    var query = "Insert into user_info(macAddress, _product_qr_code) values(" + req.body.mac_address + "," + req.body.qrcode + ")";
    con.getConnection(function (err, connection) {
        if (err) {
            res.json({ "code": 100, "status": "Error in connection database" });
            return;
...
//Added Post qr code data for admin by kashif zahid
 app.post('/postQrCode', function (req, res) {
     console.log(req.body.info.length+"----------------------------------------------------------------------------");

        for (var i = 0; i < req.body.length; i++) {


            var postQrCodequery = "Insert into product_info(product_qr_code_id) values(" + req.body.info[i].qrcode + ")";
            con.getConnection(function (err, connection) {
